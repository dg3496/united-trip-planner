...
└──────────────────────────┬──────────────────────────────┘
                ...
                (claude-sonnet-4-5 via Messages API)
```
...
3. **Edge Function** assembles a single Claude API call:
   - System prompt with persona, rules, and the inventory + user profile.
   - User messages from history + the new user message.
   - `max_tokens` ~1500, model `claude-sonnet-4-5` (or the latest Sonnet identifier at build time).
4. **Claude** returns structured JSON (see "Claude Response Contract" below).
...
Latency target end-to-end: under 5 seconds (NFR-01, FR-020). With Claude Sonnet typically responding in 2 to 4 seconds, the budget allows for ~1 second of orchestration overhead, which is tight but workable since inventory fetch is a single Postgres query.

## Claude Response Contract

The Edge Function instructs Claude to respond **only** with a JSON object matching this shape. No prose outside the JSON, no markdown fences.
...
- Every `destinationId` MUST exist in the seeded `destinations` table. The Edge Function validates this and falls back to a "no_results" response if Claude hallucinates (FR-022 grounding requirement).
...
The conversation history is passed through the `messages` array of the Anthropic API call, not in the system prompt.
...
│   └── types.ts                       # Shared TS types matching Claude contract
├── store/
...
- **Refinement Messages Reuse the Same `conversationId`.** No separate "filter" or "refine" code path. Every user input is just another message; Claude maintains intent through the message history (FR-011, FR-012).
...
- **Fallback / Circuit Breaker.** If Claude exceeds latency caps or errors out, the system shows a retry toast. The production architecture (NFR-07) specifies degrading gracefully to cached popular destinations; the prototype simplifies this to a retry message.

## Error Handling and Fallbacks

| Failure | Behavior |
|---|---|
| Claude API error or timeout | Show a toast: "I'm having trouble right now. Please try again." Do not crash the chat. |
| Claude returns invalid JSON | Edge Function logs the raw output, returns a generic "Let me try that again" message to the client, does not persist a malformed assistant turn. |
| Claude hallucinates a destination not in inventory | Edge Function strips the unknown destinations from the suggestions array. If 0 valid suggestions remain, returns a synthetic "no_results" response. |
...
- All Claude turns and user turns persist as rows in `messages`. The `metadata` JSONB column on assistant messages stores the full structured Claude response so the cards can re-render on reload.
